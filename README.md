...
| Cloud | Button |
|---|---|
| **Azure Commercial** (`portal.azure.com`) | [![Deploy to Azure](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/1-CONTRIBUTION-GUIDE/images/deploytoazure.svg)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fjstan13%2Fazure-stig-dashboard%2Fdeploy-templates%2Flatest%2Fazuredeploy.json/createUIDefinitionUri/https%3A%2F%2Fraw.githubusercontent.com%2Fjstan13%2Fazure-stig-dashboard%2Fdeploy-templates%2Flatest%2FcreateUiDefinition.json) |
| **Azure US Government** (`portal.azure.us`) | [![Deploy to Azure US Gov](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/1-CONTRIBUTION-GUIDE/images/deploytoazuregov.svg)](https://portal.azure.us/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fjstan13%2Fazure-stig-dashboard%2Fdeploy-templates%2Flatest%2Fazuredeploy.json/createUIDefinitionUri/https%3A%2F%2Fraw.githubusercontent.com%2Fjstan13%2Fazure-stig-dashboard%2Fdeploy-templates%2Flatest%2FcreateUiDefinition.json) |

> Badge art comes from `Azure/azure-quickstart-templates`, not `aka.ms/deploytoazuregovbutton` — that short link was retired and now redirects to Bing, which GitHub renders as a broken image.

> The buttons load `deploy-templates/latest/`, a branch the release workflow rewrites on every tag with the exact template it attached to that release. It exists because the Azure portal fetches the template from your browser and GitHub's release-asset downloads answer without CORS headers, which the browser blocks; `raw.githubusercontent.com` sends them. Per-release copies live alongside it at `deploy-templates/<tag>/`, and the same files stay attached to each GitHub Release.

> Each release ships a fresh `azuredeploy.json` whose container image parameters are pinned to immutable `@sha256:<digest>` references built by GitHub Actions and signed via Sigstore cosign (keyless OIDC). Deployers can independently verify the signatures before deploying — see [docs/verifying-releases.md](docs/verifying-releases.md).
